...
Ставить Windows Server, Windows Desktop и потом это настравить через GUI - крайне трудозатратное дело по времени и нервам, о чем в конце документа. Все остальное можно запустить. Помимо этого, бывали случаи, когда студент плохо не понимает, что происходит, а еще и на АРМе, так жуть вообще. 
