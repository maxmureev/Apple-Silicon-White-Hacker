...
Ребята с курса говорят, что часть лаб удалось запустить. Просто перепост сообщений из телеги. Antony, сделай пулл реквест и отредактируй инструкции сам, пожалуйста )

> Сделал 4 машины на arm, одну прямо на tryhackme. Осталась одна с переполнением буфера. Там потребуется х86. Её наверно не получиться на arm.

> Машина Mr. Robot есть на tryhackme. Её можно сделать прямо там, единственный нюанс, даётся два часа, потом отключается. 
> Машины: rsa madnes
>                   FTP-joy
>                   EEZEEPZ
> Эти машины в формате .ova , из нужно открыть в терминале мака командой tar xvf <имя загруженного файла>. Получим файл .vmdk, его запускаем на UTM( там само в qcow2 переведёт) или сделать qcow2 через терминал самому. Вот ссылка на полный пошаговый разбор. https://theenawman.medium.com/converting-sans-sec555-vmdk-to-qcow2-and-emulating-with-utm-on-apple-m2-macbook-pro-79e02f035cd
> 
> Далее для первой машины (rsa madness) нужно сделать так как указано здесь—> Суть в том, что при таком экспорте жёсткого диска, в системе создаётся другой сетевой адаптер, и он не настрон на получение ip адреса. У меня такое было при переносе вбокс машины на proxmox. В целом это решаемо. Доступ к машине > можно получать сбросом пароля рута, как я сам и делал в свое время. Его можно применить и тут. Заодно настроить сетевой адаптер после получения доступа. Его настройки будут тут /etc/network/interfaces и надо будет тому интерфейсу, который появился прописать следующее.
> 
> ```
> auto eth0
>     allow-hotplug eth0
...
> ```
> 
> только eth0 заменить на имя нового интерфейса.
> 
> https://wiki.debian.org/NetworkConfiguration
> Инструкция по тому, как сбивать пароль рута
> https://www.tecmint.com/reset-forgotten-root-password-in-debian/
> И будет в режиме host only айпишник. 
> 
> Вторая машина (FTP-joy) установить как выше описано с qcow2 в UTM но сбивать пароль рута не надо. Машина должна быть в host only, далее я в Kali запустил nmap  -sP <адрес Kali в host only и в конце .1-200>. Покажет все доступные адреса в этом промежутке ( их будет два три адреса включая саму Кали) один из них и будет адрес машины( как правило ближайший к Кали). 
> 
> Третья машина EEZEEPZ
> Также установить в UTM как по инструкции, host only, но прописать специальный MAC адрес( он указан в описании машины на странице загрузки 08:00:27:A5:A6:76) нужно просто заменить этот адрес вместо с генерированного UTM-ом. При запуске машина отобразит ip.
